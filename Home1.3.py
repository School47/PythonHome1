#### 1.3[6]. Вы пользуетесь общественным транспортом? 
# Вероятно, вы расплачивались за проезд и получали билет с номером.
# Счастливым билетом называют такой билет с шестизначным номером,
# где сумма первых трех цифр равна сумме последних трех.
# Т.е. билет с номером 385916 – счастливый, т.к. 3+8+5=9+1+6. 
# Вам требуется написать программу, которая проверяет счастливость билета.

# 	Примеры/Тесты:
# 	385916 >>> yes
# 	123456 >>> no

# ```(*)``` **Усложнение.** Вывод результат на экран сделайте одной строкой(только один print),
# для этого используйте тернарный оператор
# Синтаксис тернарного оператора:
# [если истина] if [выражение] else [если ложь]

number = int(input("Введите номер билета:"))
numberList = [int(d) for d in str(number)] #запоминаем число в виде списка цифр, чтобы потом применить срезы
print("yes") if sum(numberList[:3]) == sum(numberList[3:]) else print("no")
